...
void Syscall::addCandidate(SyscallArg *arg, int offset) {
  ...
  if (r != 0) {
    TRACE("Can't read at offset %d from argument @%.8lx", offset, arg->addr);
    return;
  }

  TRACE("Adding candidate %.8lx (closest %.8lx, offset %d, buffer %.8lx)",
...
void Syscall::actualizeCandidate(target_ulong value, target_ulong data,
  ...
  for (auto it = candidates_.begin(); it != candidates_.end(); it++) {
    ...
    TRACE("Adding a new pointer for arg @%.8lx: addr %.8lx, offset %d",
...
SyscallArg* Syscall::findClosestArgument(target_ulong targetaddr) {
  ...
  for (auto itarg = args.begin(); itarg != args.end(); itarg++) {
    ...
    for (auto itptr = arg->ptrs.begin(); itptr != arg->ptrs.end(); itptr++) {
      ...
      TRACE("Checking level-N ptr %.8lx against target %.8lx (distance %d)",
  ...
  if (closest_candidate &&
    ...
    TRACE("Candidate %.8lx is closer than other arguments (%d vs %d), "
  ...
  if (closest_arg == NULL) {
    ...
    for (auto itarg = args.begin(); itarg != args.end(); itarg++) {
      ...
      TRACE("Checking level-0 arg (addr %.8lx, value %.8lx) "
            "against target %.8lx", (*itarg)->addr, value, targetaddr);
...
SyscallArg *SyscallArg::findClosestPointer(target_ulong targetaddr) {
  ...
  for (auto it = ptrs.begin(); it != ptrs.end(); it++) {
    ...
    TRACE("Checking level-N2 ptr %.8lx against target %.8lx (distance %d)",
          tmp->addr, targetaddr, distance);
...
const std::string Syscall::to_string() const {
  ...
  oss << "Syscall " << gbl_context.guest->getSyscallName(sysno)
      << "(0x" << std::hex << sysno << "), "
...
// FIXME: This implementation sucks, change it
void SyscallArg::addRepeatedAccess(target_ulong addr, target_ulong value,
                                   int size, SyscallDirection direction,
                                   const std::vector<target_ulong> call_stack) {
  // Multi-byte memory accesses are not currently supported
  assert(size == 1);

  bool processed = false;

  // Here we traverse the list of existing repeated memory accesses, to
  // possibly merge the current one with adjacent ones
  int position = -1, i = 0;
  for (auto it = rep_accesses_.begin(); it != rep_accesses_.end(); it++, i++) {
    std::shared_ptr<MemoryAccess> curr = *it;

    // Skip non-comparable memory accesses
    if (curr->direction != direction || curr->call_stack != call_stack) {
      continue;
    }

    // If an adjacent address is found, merge the two
    ...
    // If a duplicate address is found, nothing else to do
  ...
  if (!processed) {
    MemoryAccess *memacc = new MemoryAccess(addr, size, direction);
    memacc->call_stack = call_stack;
    rep_accesses_.push_back(std::shared_ptr<MemoryAccess>(memacc));
  } else if (position != -1) {
    // We modified an existing memory access, so we must check if the modified
    // object is now adjacent to some other element
    target_ulong lower = rep_accesses_[position]->addr;
    target_ulong upper = lower + rep_accesses_[position]->size - 1;
    i = 0;
    for (auto it = rep_accesses_.begin(); it != rep_accesses_.end();
         it++, i++) {
      std::shared_ptr<MemoryAccess> curr = *it;

      // Skip non-comparable memory accesses and avoid comparing with ourselves
      if (curr->direction != direction || i == position ||
	  curr->call_stack != call_stack) {
        continue;
      }

      // In case an adjacent element is found, we modify the vector, but we
      // also immediately stop iterating
      if (upper == curr->addr) {
        rep_accesses_[position]->size += curr->size;
        rep_accesses_.erase(it);
        break;
      } else if ((curr->addr + curr->size) == lower) {
        curr->size += rep_accesses_[position]->size;
        rep_accesses_.erase(rep_accesses_.begin() + position);
        break;
      }
    }
  }
