                                          ...
                                          unsigned int flags)
{
    ...
    /* update the hidden flags */
    {
        if (seg_reg == R_CS) {
#ifdef CONFIG_QTRACE_CORE
	  /* Switch from the current protection level (CPL) to the protection
	     level of the target code segment (DPL) */
	  int dpl = (flags & DESC_DPL_MASK) >> DESC_DPL_SHIFT;
	  if ((env->hflags & HF_CPL_MASK) != dpl) {
	    qtrace_gate_cpl(env, env->hflags & HF_CPL_MASK, dpl);
	  }
#endif
#ifdef TARGET_X86_64
