...
#ifdef CONFIG_QTRACE_TAINT
                /* Intercept "XOR eax, imm" operations, used for introduce
                   run-time assertions over a register taint status */
                if (op == OP_XORL && (val == QTRACE_ASSERT_TAINTED || 
                                      val == QTRACE_ASSERT_NOT_TAINTED)) {
                  tcg_gen_qtrace_assert(cpu_regs[OR_EAX], 
                                        (val == QTRACE_ASSERT_TAINTED));
                  break;
                }
#endif
...
#ifdef CONFIG_QTRACE_TAINT
            /* Intercept "XOR reg, imm" operations */
            if (b == 0x81 && (val == QTRACE_ASSERT_TAINTED || 
                              val == QTRACE_ASSERT_NOT_TAINTED)) {
              tcg_gen_qtrace_assert(cpu_regs[opreg], 
                                    (val == QTRACE_ASSERT_TAINTED));
              break;
            }
#endif
