...
DEFHEADING()
DEFHEADING(QTrace options:)
STEXI
@table @option
ETEXI

#ifdef CONFIG_QTRACE_TRACER
DEF("qtrace-trace-disabled", 0, QEMU_OPTION_qtrace_trace_disabled,
    "-qtrace-trace-disabled\n"
    "                start with syscall tracing disabled\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-trace-disabled
@findex -qtrace-trace-disabled
Start emulation with the syscall tracing engine disabled. Can be enabled later
using QEMU monitor.
ETEXI

DEF("qtrace-log", HAS_ARG, QEMU_OPTION_qtrace_log,
    "-qtrace-log FILE\n"
    "                log QTrace messages to FILE\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-log @var{path}
@findex -qtrace-log
Save QTrace log messages to local file @var{path}.
ETEXI

DEF("qtrace-profile", HAS_ARG, QEMU_OPTION_qtrace_profile,
    "-qtrace-profile PROFILE\n"
    "                select the guest OS ('-qtrace-profile help' for list)\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-profile @var{os}
@findex -qtrace-profile
Set the guest OS to @var{os}.
ETEXI

DEF("qtrace-trace", HAS_ARG, QEMU_OPTION_qtrace_trace,
    "-qtrace-trace FILE\n"
    "                serialize syscalls to FILE\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-trace @var{path}
@findex -qtrace-trace
Make QTrace serialize system calls to local file @var{path}.
ETEXI

DEF("qtrace-syscalls", HAS_ARG, QEMU_OPTION_qtrace_syscalls,
    "-qtrace-syscalls FILTER\n"
    "                comma-separated list of syscall names to process\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-syscalls @var{filter}
@findex -qtrace-syscalls
Instruct QTrace to process only system calls with names @var{filter}.
ETEXI

DEF("qtrace-process", HAS_ARG, QEMU_OPTION_qtrace_process,
    "-qtrace-process NAME\n"
    "                name of the process to trace\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-process @var{name}
@findex -qtrace-process
Instruct QTrace to process only process with name @var{name}.
ETEXI

DEF("qtrace-foreign", 0, QEMU_OPTION_qtrace_foreign, \
    "-qtrace-foreign\n"
    "                enable foreign pointers tracking\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-foreign
@findex -qtrace-foreign
Enable tracking of foreign function pointers, i.e., function pointers
that do not belong to system calls arguments.
ETEXI

DEF("qtrace-rep-accesses", 0, QEMU_OPTION_qtrace_rep_accesses,
    "-qtrace-rep-accesses\n"
    "                track repeated memory accesses (i.e., possible races)\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-rep-accesses
@findex -qtrace-rep-accesses
Track repeated memory accesses to the same memory location, during the execution
of the same system calls. These *may* be symptom of a race condition in the
guest kernel.
ETEXI
#endif

#ifdef CONFIG_QTRACE_TAINT
DEF("qtrace-taint-disabled", 0, QEMU_OPTION_qtrace_taint_disabled,
    "-qtrace-taint-disabled\n"
    "                start with taint-tracking disabled\n",
    QEMU_ARCH_ALL)
STEXI
@item -qtrace-taint-disabled
@findex -qtrace-taint-disabled
Start emulation with the taint-propagation engine disabled. Can be enabled
later using QEMU monitor.
ETEXI
#endif
#endif
